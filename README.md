...
- **Firmware (ESPHome controller):** Install ESPHome, connect the ESP32-C3 over USB-C, and run `esphome run firmware/src/controller/esphome/tankpro.yaml` to compile and flash. On first boot the device exposes `SmartRV-TankPro-Setup` (password `changeme`) for captive-portal provisioning; then adopt in ESPHome/Home Assistant. Wiring guidance is in `docs/wiring-and-installation.md`; LED state legend and button usage are in `docs/getting-started.md`.
- **Display firmware (Cheap Yellow Display):** The paired CYD client lives in `firmware/src/display/CYD` (`env:cyd`). Build/flash with PlatformIO or `esptool.py` using the binaries under `.pio/build/cyd/`. User-friendly install/update steps for a store-bought Cheap Yellow Display are in `docs/display-firmware-installation.md`; feature overview is in `docs/display-firmware.md`.
