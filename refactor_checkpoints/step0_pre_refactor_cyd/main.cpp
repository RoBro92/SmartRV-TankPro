...
void mark_setup_complete_and_persist();
void mark_setup_complete_direct();
...
static String current_mac_string() {
    uint8_t mac[6] = {0};
    esp_read_mac(mac, ESP_MAC_WIFI_STA);
    char buf[18];
...
static void handle_root_route() {
    const String mac = current_mac_string();
    String page = R"HTML(
<!DOCTYPE html><html><head>
...
<p><strong>Device ID:</strong> )HTML";
    page += device_id;
    page += R"HTML(<br><strong>Firmware:</strong> )HTML";
    page += cyd_state.firmware_version;
    page += R"HTML(<br><strong>MAC:</strong> )HTML";
    page += mac;
    page += R"HTML(</p>
