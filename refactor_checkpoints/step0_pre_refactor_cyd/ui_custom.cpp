...
// Legacy boot-assignment widgets were removed from the regenerated UI. Keep null
// placeholders so existing logic compiles and simply no-ops when those elements
// are absent.
static lv_obj_t *ui_bootAssignConfirmButton = nullptr;
static lv_obj_t *ui_bootAssignFreshButton = nullptr;
static lv_obj_t *ui_bootAssignWasteButton = nullptr;
static lv_obj_t *ui_bootAssignDeviceList = nullptr;
static lv_obj_t *ui_bootAssignCancelButton = nullptr;
static lv_obj_t *ui_bootOverlayAssignRole = nullptr;
static lv_obj_t *ui_overlayBootDirect = nullptr;
static lv_obj_t *ui_buttonbootdirectback = nullptr;
static lv_obj_t *ui_buttonbootdirectscan = nullptr;
...
static void show_boot_direct(lv_event_t *e) {
    LV_UNUSED(e);
    // Skip onboarding/persistence: just land on the home screen for direct use.
    stop_wifi_onboarding();
    _ui_screen_change(&ui_home, LV_SCR_LOAD_ANIM_NONE, 0, 0, NULL);
    cyd_state_apply_to_home_screen();
}
...
static uint32_t cyd_reset_press_start_ms = 0;
static bool cyd_reset_fired = false;
static void cydsettings_factory_reset_handler(lv_event_t *e) {
    lv_event_code_t code = lv_event_get_code(e);
    if (code == LV_EVENT_PRESSED) {
        cyd_reset_press_start_ms = lv_tick_get();
        cyd_reset_fired = false;
    } else if (code == LV_EVENT_PRESSING && cyd_reset_press_start_ms != 0) {
        if (!cyd_reset_fired && lv_tick_elaps(cyd_reset_press_start_ms) >= 2000) {
            cyd_reset_fired = true;
            cydsettings_hide_factory_overlay(nullptr);
            factory_reset_and_reboot();
        }
    } else if (code == LV_EVENT_RELEASED || code == LV_EVENT_PRESS_LOST) {
        cyd_reset_press_start_ms = 0;
    }
}
...
static void waste_start_drain(lv_event_t *e) {
    LV_UNUSED(e);
    controller_link_send_command(TANK_ROLE_WASTE, "drain");
}
...
void ui_register_custom_actions() {
    ...
    if (ui_cydsettingsResetButton) {
        ...
        lv_obj_add_event_cb(ui_cydsettingsResetButton, cydsettings_factory_reset_handler, LV_EVENT_ALL, NULL);
    }
    ...
    if (ui_WasteDrainButton) {
        lv_obj_add_flag(ui_WasteDrainButton, LV_OBJ_FLAG_CLICKABLE);
        lv_obj_add_event_cb(ui_WasteDrainButton, waste_start_drain, LV_EVENT_CLICKED, NULL);
    }
